...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        """
        Remove words that don't match variable lengths.
        """
        for var in self.crossword.variables:
    ...
    def ac3(self, arcs=None):
        ...
        while queue:
            x, y = queue.pop(0)
            if self.revise(x, y):
                if not self.domains[x]:  # If a domain becomes empty, failure
                    return False
    ...
    def order_domain_values(self, var, assignment):
        """
        Order values by least-constraining heuristic.
        """
        def count_ruled_out(word):
            ...
            for neighbor in self.crossword.neighbors(var):
                if neighbor not in assignment:
                    ...
                    count += sum(1 for neighbor_word in self.domains[neighbor]
