...
class RulesBuilder:
    ...
    def get_beginning_chip(self):
        ...
        # 中间不下注的人的 bet 都是0
        midbets = []
        for _i in range(self.players - 2):
            midbets.append(0)
...
class TreeBuilder:
    ...
    def gen_km_json(self, json_file, path_prefix=None, limit=np.inf, ret_json=False):
        """
        利用百度脑图的格式可视化决策树
        """
    ...
    def __gen_km_json(self, root, depth, limit, parent=None):
        ...
        text += "\nround: {}".format(
            {1: "preflop", 2: "flop", 3: "turn", 4: "river"}[root.betting_round]
        )
        ...
        if root.showdown and root.betting_round < 4:
            one_json["meta"]["node_type"] = node_type
            one_json["meta"]["round"] = {1: "preflop", 2: "flop", 3: "turn", 4: "river"}[
                root.betting_round + 1
            ]
...
class FiveCardTexasTreeBuilder(TreeBuilder):
    def build_tree(self):
        ...
        root = ActionNode(
            None,
            committed=self.rule.get_beginning_chip(),
            players=players,
            player=player,
            last_action="begin",
            bet_history=[],
    ...
    def get_possible_betting_sizes(self, root, player, next_player, typeofbet, rule):
        ...
        if committed[player] == sb:
            ...
            possible_amounts = [
                i for i in possible_amounts if i <= self.rule.stack - committed[player]
            ]
    ...
    def build_action(self, root):
        ...
        for one_action in possible_actions:
            if one_action == "check":
                ...
                if (
                    root.last_action == "call" and root.betting_round == 1
                ) or checksum >= check_limit - 1:
                    # 双方均check的话,进入摊派阶段
                    # check 只有最有一轮（river）的时候才是摊派，否则都是应该进入下一轮的
                    if root.betting_round == self.rule.rounds:
                        nextnode = ShowdownNode(
                            root,
                            committed=committed,
                            players=players,
                            player=next_player,
                            last_action=one_action,
                            bet_history=[*root.bet_history, one_action],
                        )
                    else:
                        # 第二轮开始先决定的就是deal card player了,所以这里的player 不是 next_player 而是player 1
                        nextnode = DealCardNode(
                            root,
                            committed=committed,
                            players=players,
                            player=1,
                            last_action=one_action,
                            bet_history=[*root.bet_history, one_action],
                            betting_round=root.betting_round + 1,
                        )
                ...
                betting_sizes = self.get_possible_betting_sizes(
                    root, player, next_player, "bet", self.rule
                )
                for one_betting_size in betting_sizes:
                    ...
                    nextnode = ActionNode(
                        ...
                        players=players,
                        player=next_player,
                        last_action=one_action_bet,
                        bet_history=[*root.bet_history, one_action_bet],
                    )
                    self.__build(nextnode)
                ...
                elif root.betting_round == self.rule.rounds or np.any(
                    self.rule.stack - np.asarray(root.committed) <= 0
                ):
                    nextnode = ShowdownNode(
                        root,
                        committed=committed,
                ...
                betting_sizes = self.get_possible_betting_sizes(
                    root, player, next_player, "raise", self.rule
                )
                ...
                for one_betting_size in betting_sizes:
                    ...
                    nextnode = ActionNode(
                        ...
                        players=players,
                        player=next_player,
                        last_action=one_action_raise,
                        bet_history=[*root.bet_history, one_action_raise],
                    )
                    self.__build(nextnode)
...
class PartGameTreeBuilder(FiveCardTexasTreeBuilder):
    ...
    def build_tree(self):
        ...
        root = ActionNode(
            None,
            committed=self.rule.current_commit,
            players=players,
            player=player,
...
class Node:
    def __init__(
        ...
        self.betting_round = (
            ...
            else (parent.betting_round if parent is not None else None)
        )
...
class ChanceNode(Node):
    def __init__(
        ...
        super().__init__(
            parent, committed, players, player, bet_history, betting_round=betting_round
        )
...
class ShowdownNode(ActionNode):
    def __init__(
        ...
        super().__init__(
            parent, committed, players, player, bet_history, betting_round=betting_round, **kwargs
        )
        self.last_action = kwargs["last_action"]
        if parent:
            self.parent.add_child(self, self.last_action)
